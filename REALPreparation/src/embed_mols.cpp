...
int main(int argc, char* argv[])
{
  ...
  std::cout << "Processing the vector ..." << endl;
      ...
      for (size_t counter = chunk_beg; counter < chunk_end; ++counter)
      {
        boost_ifstream pdbqt_ifs(pdbqts_vector[counter]);
					...
					if (line.find("SMILES:") != string::npos)
          {
              ...
              break;
					...
					else
					{
						stringstream iss, oss;
						{
							...
							for (string line; getline(pdbqt_ifs, line); iss << line << endl);
							OBConversion obconv(&iss, &oss);
							OBMol mol;
							obconv.SetInAndOutFormats("pdbqt", "sdf");
							obconv.Read(&mol);
							obconv.Write(&mol);
							cout << oss.str() << endl;
						}
						SDMolSupplier sup(&oss, true, true, false, true);
						// read from sdf
						try
						{
							const unique_ptr<ROMol> mol_ptr(sup.next());
							const unique_ptr<ROMol> molHs_ptr(addHs(*mol_ptr));
							ROMol& molecule = *molHs_ptr;
							const auto confIds = EmbedMultipleConfs(molecule, 4, params);
							if (confIds.empty()) 
								break;
							if (confIds.size() == 4)
							{
								realfprop[0] = calcExactMW(molecule);
								realfprop[1] = calcClogP(molecule);
								realfprop[2] = calcTPSA(molecule);
								realfprop[3] = calcLabuteASA(molecule);

								realiprop[0] = molecule.getNumHeavyAtoms();
								realiprop[1] = calcNumHBD(molecule);
								realiprop[2] = calcNumHBA(molecule);
								realiprop[3] = calcNumRotatableBonds(molecule);
								realiprop[4] = calcNumRings(molecule);

								{
									lock_guard<mutex> lock(mu);
									std::cout << "Iteration N° " << counter << " And generated ligands: " << lig_count <<  endl;
									std::cout << "Processing file: " << pdbqts_vector[counter] << endl;
									// std::cout << confIds.size() << " Conformers of " << compound << " : " << smiles << " are successfully generated!" << endl;
									lig_count++;
									for (const auto confId : confIds)
									{
										writer.write(molecule, confId);
										usrcat_features = usrcat_generator(molecule, confId);
			...
			cnt.increment();
				/*--------------------- ADDED --------------------*/
				// read pdbqt and write to sdf
				// boost_ifstream ifs(pdbqts_vector[counter]);
    });
  }
  cnt.wait();
  const auto finished = system_clock::now();
  const auto runtime = (finished - started).count() * 1e-9;
  std::cout << "process finished in " << setprecision(3) << runtime << " seconds" << endl; 
}
