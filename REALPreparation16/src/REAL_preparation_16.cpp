...
USRCAT_DESCRIPTORS usrcat_generator(ROMol& mol, int index) {
  ...
  for (const auto& subset : subsets) {
    ...
    for (size_t ref = 0; ref < num_references; ++ref) {
      ...
      for (const auto e : m) {
        // write the features in 2 bytes format
        int16_t val8_t = e * 1000;
        features[qo++] = val8_t;
      }
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  USRCAT_DESCRIPTORS usrcat_features;
